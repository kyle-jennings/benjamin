...
/**
 * Array of templates and their settings
 *
 * @param  boolean $use_widget_areas [description]
 * @param  boolean $add_default      [description]
 *
...
function benjamin_the_template_list( $use_widget_areas = false, $add_default = false ) {
	...
	$desc_warning = __(
		'The layout settings and widgets on this template are not available
	unless the template is activated in the customizer.',
		'benjamin'
	);
	...
	$templates = array(
		...
		'template-3' => array(
			...
			'description'        => sprintf(
				/* translators:  warning about activating the widget areas. */
				__(
					'This is just an extra page template, use this
	...
	if ( $add_default === true ) {
		$templates = array(
			BENJAMIN_DEFAULT_TEMPLATE => array(
