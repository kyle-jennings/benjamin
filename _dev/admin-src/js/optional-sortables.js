jQuery(function($) {
  ...
  var $sortables = $('.js--sortables');
  var $sortableList = $sortables.find('.js--sortables-list');
  var siblingsName = $sortables.find('.'+$sortableList.data('sortable-group'));
  var id = $sortableList.data('sortable-group').replace('_control', '_setting');
  var $active = $sortables.find('.js--sortables-active');
  var $field = $sortables.find('input[type="hidden"]');
